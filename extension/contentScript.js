chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'autoFillAuthors') {
    const profileKey = request.profile || 'profile'; 
...
function fillAuthorsSequentially(authors) {
  ...
  function processNextAuthor() {
    if (index >= authors.length) {
      console.log('All author entries have been successfully filled.');
      alert('All authors have been successfully auto-filled!');
      return;
    }
